...
async def test_interpreter_exec_posts_code_and_timeout_to_sidecar():
    manager, fake_client = _manager_with_stubbed_sidecar(
        {"stdout": "", "result": "42", "error": None, "truncated": False}
    )

    result = await manager.interpreter_exec(
    ...
    assert fake_client.post_calls == [
        ("/interpreter/exec", {"code": "40 + 2", "timeout": 15})
    ]


async def test_interpreter_exec_defaults_timeout_to_30_seconds():
    manager, fake_client = _manager_with_stubbed_sidecar(
        {"stdout": "", "result": None, "error": None, "truncated": False}
    )

    await manager.interpreter_exec(session_id="session-1", code="1")

    assert fake_client.post_calls == [("/interpreter/exec", {"code": "1", "timeout": 30})]


async def test_interpreter_reset_posts_to_sidecar_with_no_body():
    manager, fake_client = _manager_with_stubbed_sidecar({"status": "reset"})

    result = await manager.interpreter_reset(session_id="session-1")

    assert result == {"status": "reset"}
    assert fake_client.post_calls == [("/interpreter/reset", {})]
