...
-- that there is no job queue/scheduler/fleet abstraction in src/boxxkite/
...
since docs/ and src/boxxkite/ are no longer siblings after the
...
def test_no_job_queue_or_scheduler_exists_in_manager_source():
    ...
    manager_src = (REPO_ROOT / "src" / "boxxkite" / "manager.py").read_text()
    warm_pool_src = (REPO_ROOT / "src" / "boxxkite" / "_manager_warm_pool.py").read_text()
