"""Parity test: src/boxxkite/sidecar_auth.py vs sidecar/main.py's duplicated constants.

sidecar/main.py is a separately deployed service that intentionally does not
depend on the `boxxkite` package, so it re-declares the env var name and
