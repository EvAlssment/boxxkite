...
def _normalize_to_ws_origin(control_plane_url: str) -> str:
    ...
    `BOXXKITE_CONTROL_PLANE_URL`-shaped value used elsewhere in this repo's
    ...
    raise BastionConfigError(
        f"BOXXKITE_BASTION_CONTROL_PLANE_URL must start with http://, https://, ws://, or wss:// (got: {control_plane_url!r})"
    )
...
class BastionSettings:
    ...
    def from_env() -> "BastionSettings":
        raw_control_plane_url = os.environ.get("BOXXKITE_BASTION_CONTROL_PLANE_URL", "").strip()
        if not raw_control_plane_url:
            raise BastionConfigError("BOXXKITE_BASTION_CONTROL_PLANE_URL is required and was not set")
        return BastionSettings(
            control_plane_ws_base_url=_normalize_to_ws_origin(raw_control_plane_url),
            listen_host=os.environ.get("BOXXKITE_BASTION_LISTEN_HOST", "0.0.0.0"),  # noqa: S104
            listen_port=int(os.environ.get("BOXXKITE_BASTION_LISTEN_PORT", "2222")),
            host_key_path=os.environ.get("BOXXKITE_BASTION_HOST_KEY_PATH") or None,
            max_connections_per_host=int(os.environ.get("BOXXKITE_BASTION_MAX_CONNECTIONS_PER_HOST", "10")),
            login_timeout_seconds=float(os.environ.get("BOXXKITE_BASTION_LOGIN_TIMEOUT_SECONDS", "30")),
        )
