...
§4) -- config parsing (mirrors BOXXKITE_BASE_IMAGE_REFS_RAW) and the
...
def test_default_catalog_has_an_entry_for_every_shipped_provider():
    catalog = settings.BOXXKITE_MCP_CATALOG
...
def test_every_schema_literal_value_has_a_matching_catalog_entry(catalog_id: str):
    ...
    value must resolve against the default BOXXKITE_MCP_CATALOG_RAW, or a
...
def test_catalog_raw_parsing_handles_trailing_commas_and_whitespace():
    original = settings.BOXXKITE_MCP_CATALOG_RAW
    try:
        settings.BOXXKITE_MCP_CATALOG_RAW = " slack = mcp.slack.com , notion=mcp.notion.com ,"
        catalog = settings.BOXXKITE_MCP_CATALOG
        assert catalog == {"slack": "mcp.slack.com", "notion": "mcp.notion.com"}
    finally:
        settings.BOXXKITE_MCP_CATALOG_RAW = original
