...
def test_build_pvc_spec_uses_configured_storage_class(monkeypatch):
    ...
    monkeypatch.setattr(settings, "BOXXKITE_VOLUME_STORAGE_CLASS", "fast-ssd")
...
def test_build_pvc_spec_names_are_deterministic_and_dns_safe():
    ...
    assert name == spec["_boxxkite_pvc_name"]
...
def test_build_pvc_spec_labels_namespace_by_account_and_volume():
    ...
    assert labels["boxxkite.dev/account-id"] == "acct-1"
    assert labels["boxxkite.dev/volume-id"] == "vol-1"
...
async def test_fake_volume_provisioner_tracks_deprovision_calls():
    ...
    await provisioner.deprovision(pvc_name="boxxkite-vol-abc")

    assert provisioner.deprovisioned == ["boxxkite-vol-abc"]
