"""Background task enforcing BOXXKITE_MAX_SESSION_MINUTES server-side,
...
than the cap on a fixed interval (BOXXKITE_SESSION_REAPER_INTERVAL_SECONDS).

Also reaps the public demo playground's sessions (issue #103,
demo_account.py) on their own, much shorter BOXXKITE_DEMO_LIFETIME_MINUTES
cutoff, in addition to the global BOXXKITE_MAX_SESSION_MINUTES cutoff every
other account gets above -- a demo session's pod is already killed by its
own K8s activeDeadlineSeconds at BOXXKITE_DEMO_LIFETIME_MINUTES, but nothing
else would mark its `sandbox_sessions` row destroyed until the much longer
global cutoff elapses, silently holding a BOXXKITE_DEMO_MAX_CONCURRENT slot
...
async def _reap_once(manager) -> None:
    ...
    cutoff = now - timedelta(minutes=settings.BOXXKITE_MAX_SESSION_MINUTES)
    ...
    async with session_factory() as db:
        ...
        if settings.BOXXKITE_DEMO_PLAYGROUND_ENABLED:
            demo_account = await AccountRepository(db).get_by_email(DEMO_ACCOUNT_EMAIL)
            if demo_account is not None:
                demo_cutoff = now - timedelta(minutes=settings.BOXXKITE_DEMO_LIFETIME_MINUTES)
...
async def run_reaper_loop(manager, *, stop_event: asyncio.Event) -> None:
    interval = settings.BOXXKITE_SESSION_REAPER_INTERVAL_SECONDS
