...
CREATE TABLE IF NOT EXISTS memory_records (
	...
	PRIMARY KEY (id),
	CONSTRAINT uq_memory_records_account_scope_kind_hash UNIQUE (account_id, scope, kind, content_hash),
	FOREIGN KEY(account_id) REFERENCES accounts (id) ON DELETE CASCADE,
	FOREIGN KEY(superseded_by_id) REFERENCES memory_records (id) ON DELETE SET NULL
);

CREATE INDEX IF NOT EXISTS ix_memory_records_account_expires ON memory_records (account_id, expires_at);

CREATE INDEX IF NOT EXISTS ix_memory_records_account_id ON memory_records (account_id);

CREATE INDEX IF NOT EXISTS ix_memory_records_account_scope_kind ON memory_records (account_id, scope, kind);

CREATE INDEX IF NOT EXISTS ix_memory_records_account_scope_updated ON memory_records (account_id, scope, updated_at);

CREATE INDEX IF NOT EXISTS ix_memory_records_account_superseded ON memory_records (account_id, superseded_at);

CREATE INDEX IF NOT EXISTS ix_memory_records_content_hash ON memory_records (content_hash);

CREATE INDEX IF NOT EXISTS ix_memory_records_source_session_id ON memory_records (source_session_id);
...
CREATE TABLE IF NOT EXISTS memory_embeddings (
	...
	updated_at TIMESTAMP WITH TIME ZONE NOT NULL,
	PRIMARY KEY (id),
	CONSTRAINT uq_memory_embeddings_memory_model UNIQUE (memory_id, model_id),
	FOREIGN KEY(account_id) REFERENCES accounts (id) ON DELETE CASCADE,
	FOREIGN KEY(memory_id) REFERENCES memory_records (id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS ix_memory_embeddings_account_id ON memory_embeddings (account_id);

CREATE INDEX IF NOT EXISTS ix_memory_embeddings_account_model ON memory_embeddings (account_id, model_id);

CREATE INDEX IF NOT EXISTS ix_memory_embeddings_default_hnsw ON memory_embeddings USING hnsw (CAST(embedding AS VECTOR(768)) vector_cosine_ops) WHERE model_id = 'qwen3-embedding-0.6b-768-v1' AND dimensions = 768;

CREATE INDEX IF NOT EXISTS ix_memory_embeddings_memory_id ON memory_embeddings (memory_id);

CREATE INDEX IF NOT EXISTS ix_memory_embeddings_content_hash ON memory_embeddings (content_hash);
...
CREATE TABLE IF NOT EXISTS memory_relations (
	...
	created_at TIMESTAMP WITH TIME ZONE NOT NULL,
	PRIMARY KEY (id),
	CONSTRAINT uq_memory_relations_edge UNIQUE (account_id, source_memory_id, target_memory_id, relation_type),
	FOREIGN KEY(account_id) REFERENCES accounts (id) ON DELETE CASCADE,
	FOREIGN KEY(source_memory_id) REFERENCES memory_records (id) ON DELETE CASCADE,
	FOREIGN KEY(target_memory_id) REFERENCES memory_records (id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS ix_memory_relations_account_id ON memory_relations (account_id);

CREATE INDEX IF NOT EXISTS ix_memory_relations_account_source ON memory_relations (account_id, source_memory_id);

CREATE INDEX IF NOT EXISTS ix_memory_relations_account_target ON memory_relations (account_id, target_memory_id);
