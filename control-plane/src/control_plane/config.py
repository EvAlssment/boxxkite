...
class Settings(BaseSettings):
    ...
    BOXXKITE_METRICS_ENABLED: bool = True
    ...
    BOXXKITE_JSON_LOGS: bool | None = None
    ...
    # follow this repo's standard convention (BOXXKITE_IMAGE_BUILDER_ENABLED,
    # BOXXKITE_VOLUMES_ENABLED, enable_git_tools, ...): off by default, an
    # explicit opt-in required. See routers/auth.py's module docstring for
    # the full behavior of each flag.
    BOXXKITE_REFRESH_TOKENS_ENABLED: bool = False
    ...
    BOXXKITE_PASSWORD_RESET_ENABLED: bool = False
    PASSWORD_RESET_TOKEN_TTL_MINUTES: int = 30
    BOXXKITE_EMAIL_VERIFICATION_ENABLED: bool = False
    ...
    # than inheriting BOXXKITE_AUTH_RATE_LIMIT_PER_MINUTE's signup/login
    ...
    BOXXKITE_PASSWORD_RESET_RATE_LIMIT_PER_MINUTE: int = 5
    BOXXKITE_EMAIL_VERIFICATION_RATE_LIMIT_PER_MINUTE: int = 5
    BOXXKITE_REFRESH_RATE_LIMIT_PER_MINUTE: int = 20

    # API keys are presented as `bxk_live_<random>` so a leaked key is
    # greppable in logs/history and instantly identifiable as a boxxkite
    ...
    BOXXKITE_ORG_INVITE_TTL_HOURS: int = 168
    ...
    BOXXKITE_MCP_OAUTH_ENABLED: bool = True
    ...
    # story.
    BOXXKITE_MCP_ACCESS_TOKEN_TTL_MINUTES: int = 15
    ...
    BOXXKITE_MCP_REFRESH_TOKEN_TTL_DAYS: int = 30
    ...
    BOXXKITE_MCP_AUTH_CODE_TTL_SECONDS: int = 60
    BOXXKITE_MCP_LOGIN_SESSION_TTL_MINUTES: int = 15
    ...
    # bucket, same rationale BOXXKITE_IMAGE_BUILD_RATE_LIMIT_PER_MINUTE
    # already has for a different open-to-anyone-ish endpoint.
    BOXXKITE_OAUTH_DCR_RATE_LIMIT_PER_MINUTE: int = 10
    ...
    # as BOXXKITE_MCP_OAUTH_ENABLED above (the login-CSRF/session-fixation
    ...
    # BOXXKITE_BASE_IMAGE_REFS already has for a different feature.
    BOXXKITE_SOCIAL_LOGIN_ENABLED: bool = True
    ...
    # convention as BOXXKITE_SOCIAL_LOGIN_ENABLED/BOXXKITE_MCP_OAUTH_ENABLED.
    ...
    BOXXKITE_ENTERPRISE_SSO_ENABLED: bool = False
    ...
    # pending review" convention as BOXXKITE_ENTERPRISE_SSO_ENABLED/
    # BOXXKITE_SOCIAL_LOGIN_ENABLED/BOXXKITE_MCP_OAUTH_ENABLED. Deliberately
    # its OWN flag, independent of BOXXKITE_ENTERPRISE_SSO_ENABLED -- the
    ...
    BOXXKITE_SCIM_PROVISIONING_ENABLED: bool = False
    ...
    # BOXXKITE_SCIM_PROVISIONING_ENABLED and this are set, mirroring
    ...
    # inherit a shared default" treatment BOXXKITE_OAUTH_DCR_RATE_LIMIT_PER_MINUTE/
    # BOXXKITE_WEBHOOK_RATE_LIMIT_PER_MINUTE already have. Sized higher than
    ...
    BOXXKITE_SCIM_WEBHOOK_RATE_LIMIT_PER_MINUTE: int = 60
    ...
    # (no trailing slash), e.g. "https://api.your-boxxkite-host.example.com".
    ...
    # `{BOXXKITE_DASHBOARD_URL}/dashboard/oauth-callback` after the provider
    ...
    BOXXKITE_GLOBAL_MAX_CONCURRENT_SANDBOXES: int = 20
    ...
    BOXXKITE_MAX_SANDBOX_SIZE: str = "medium"
    ...
    BOXXKITE_MAX_SANDBOX_STORAGE_GB: float = 20.0

    BOXXKITE_MAX_ALLOWLIST_RULES: int = 50
    BOXXKITE_MAX_ALLOWLIST_PATTERN_LENGTH: int = 200
    ...
    BOXXKITE_RATE_LIMIT_BACKEND: str = "memory"
    # In-memory, per-process sliding-window cap on /v1/auth/signup and
    # /v1/auth/login requests per source IP. See BOXXKITE_RATE_LIMIT_BACKEND
    # above for the multi-replica caveat.
    BOXXKITE_AUTH_RATE_LIMIT_PER_MINUTE: int = 10
    # Same "memory" vs "postgres" choice as BOXXKITE_RATE_LIMIT_BACKEND, but
    ...
    # portable to SQLite, unlike BOXXKITE_RATE_LIMIT_BACKEND's counter-table
    ...
    BOXXKITE_SANDBOX_RATE_LIMIT_PER_MINUTE: int = 120
    ...
    BOXXKITE_SANDBOX_LIFECYCLE_RATE_LIMIT_PER_MINUTE: int = 20
    ...
    BOXXKITE_SNAPSHOT_RATE_LIMIT_PER_MINUTE: int = 10
    ...
    # bucket.
    BOXXKITE_PREVIEW_RATE_LIMIT_PER_MINUTE: int = 300
    ...
    # is attempted.
    BOXXKITE_MAX_SNAPSHOTS_PER_ACCOUNT: int = 10
    ...
    # src/boxxkite/manager.py and sidecar/main.py) but with a SNAPSHOT_
    ...
    # (src/boxxkite/manager.py's STORAGE_S3_KMS_KEY_ID/
    ...
    SNAPSHOT_STORAGE_AZURE_CONTAINER: str = "boxxkite-sandbox"
    ...
    BOXXKITE_ALLOW_INSECURE_LOCAL_KMS: bool = False
    ...
    # session's lifetime (bounded anyway by BOXXKITE_MAX_SESSION_MINUTES),
    ...
    # BOXXKITE_MAX_SNAPSHOTS_PER_ACCOUNT's rationale (unbounded rows per
    # account is a quota gap, not just an API nicety).
    BOXXKITE_MAX_SECRETS_PER_ACCOUNT: int = 50
    ...
    # BOXXKITE_WEBHOOK_RATE_LIMIT_PER_MINUTE's "own, more conservative bucket"
    # rationale.
    BOXXKITE_SECRET_RATE_LIMIT_PER_MINUTE: int = 10
    ...
    BOXXKITE_TAKEOVER_TOKEN_TTL_SECONDS: int = 30
    # TTL for the short-lived, single-use token
    ...
    # BOXXKITE_TAKEOVER_TOKEN_TTL_SECONDS above (only needs to survive the
    ...
    BOXXKITE_SANDBOX_CREATE_TOKEN_TTL_SECONDS: int = 90
    ...
    # BOXXKITE_IMAGE_BUILDER_ENABLED/BOXXKITE_AGENT_PTY_ENABLED: this durably
    ...
    BOXXKITE_TAKEOVER_RECORDING_ENABLED: bool = False
    ...
    # BOXXKITE_TAKEOVER_TOKEN_TTL_SECONDS above (this token only needs to
    # survive the moment between minting and completing the `WS .../desktop`
    # upgrade).
    BOXXKITE_DESKTOP_TOKEN_TTL_SECONDS: int = 30
    # The control-plane-side kill switch for `WS .../desktop` and
    # `POST .../desktop-token` -- independent of the sidecar's own
    # BOXXKITE_DESKTOP_ENABLED env var baked into pods. Both routes 404/close
    ...
    # BOXXKITE_TAKEOVER_RECORDING_ENABLED/BOXXKITE_IMAGE_BUILDER_ENABLED.
    BOXXKITE_DESKTOP_ENABLED: bool = False
    # The control-plane-side kill switch for the /lsp/* routes -- independent
    # of the sidecar's own BOXXKITE_LSP_ENABLED env var baked into pods. Routes
    ...
    # default, same "new capability stays opt-in" convention as
    # BOXXKITE_DESKTOP_ENABLED/BOXXKITE_TAKEOVER_RECORDING_ENABLED.
    BOXXKITE_LSP_ENABLED: bool = False
    ...
    # pod. Off by default (BOXXKITE_IMAGE_BUILDER_ENABLED=false) so a
    # deployment must explicitly turn this feature on.
    BOXXKITE_IMAGE_BUILDER_ENABLED: bool = False
    ...
    BOXXKITE_MAX_IMAGES_PER_ACCOUNT: int = 10
    ...
    BOXXKITE_IMAGE_BUILD_CACHE_HOURS: float = 24.0
    ...
    BOXXKITE_IMAGE_BUILD_RATE_LIMIT_PER_MINUTE: int = 3
    # Cluster-wide ceiling on simultaneously in-flight builds (status
    # "queued" or "building") across ALL accounts combined -- mirrors
    # BOXXKITE_GLOBAL_MAX_CONCURRENT_SANDBOXES's rationale: enough accounts
    # each near their own per-account cap (BOXXKITE_MAX_IMAGES_PER_ACCOUNT)
    ...
    BOXXKITE_GLOBAL_MAX_CONCURRENT_IMAGE_BUILDS: int = 5
    ...
    BOXXKITE_IMAGE_BUILD_CPU_REQUEST: str = "500m"
    BOXXKITE_IMAGE_BUILD_CPU_LIMIT: str = "2"
    BOXXKITE_IMAGE_BUILD_MEMORY_REQUEST: str = "1Gi"
    BOXXKITE_IMAGE_BUILD_MEMORY_LIMIT: str = "4Gi"
    ...
    # hang.
    BOXXKITE_IMAGE_BUILD_TIMEOUT_SECONDS: int = 1200
    ...
    BOXXKITE_IMAGE_REGISTRY_PREFIX: str = "registry.internal/boxxkite-images"
    # Namespace the builder Job/ConfigMap are created in. Same env var and
    # same default as `src/boxxkite/manager.py`'s SANDBOX_NAMESPACE
    ...
    BOXXKITE_IMAGE_BUILD_WAIT_TIMEOUT_SECONDS: float = 900.0
    ...
    # the K8s API server.
    BOXXKITE_IMAGE_BUILD_POLL_INTERVAL_SECONDS: float = 3.0
    ...
    BOXXKITE_IMAGE_SCAN_BLOCK_SEVERITIES_RAW: str = "critical,high"

    @property
    def BOXXKITE_IMAGE_SCAN_BLOCK_SEVERITIES(self) -> list[str]:
        return [s.strip().lower() for s in self.BOXXKITE_IMAGE_SCAN_BLOCK_SEVERITIES_RAW.split(",") if s.strip()]
    ...
    BOXXKITE_IMAGE_SCAN_REQUIRED: bool = True
    # Wall-clock cap on one `trivy image` invocation -- mirrors
    # BOXXKITE_IMAGE_BUILD_WAIT_TIMEOUT_SECONDS's rationale: a scan that hangs
    ...
    # src/boxxkite/manager.py) -- this is still "N reviewed bases," never a
    ...
    BOXXKITE_BASE_IMAGE_REFS_RAW: str = (
        "boxxkite-default=ghcr.io/evalssment/boxxkite-sandbox:latest,"
        "boxxkite-minimal=ghcr.io/evalssment/boxxkite-sandbox-minimal:latest,"
        "boxxkite-node=ghcr.io/evalssment/boxxkite-sandbox-node:latest,"
        "boxxkite-go=ghcr.io/evalssment/boxxkite-sandbox-go:latest,"
        "boxxkite-nextjs=ghcr.io/evalssment/boxxkite-sandbox-nextjs:latest,"
        "boxxkite-rust=ghcr.io/evalssment/boxxkite-sandbox-rust:latest"
    )

    @property
    def BOXXKITE_BASE_IMAGE_REFS(self) -> dict[str, str]:
        refs: dict[str, str] = {}
        for entry in self.BOXXKITE_BASE_IMAGE_REFS_RAW.split(","):
            entry = entry.strip()
            if not entry:
                continue
    ...
    # Mirrors BOXXKITE_BASE_IMAGE_REFS_RAW's exact pattern immediately
    # above, applied to hostnames instead of image digests: a small,
    # boxxkite-maintained allowlist a POST /v1/mcp-connections request's
    ...
    # enforces for BOXXKITE_BASE_IMAGE_REFS/schemas.py's `base` Literal.
    #
    ...
    @property
    def BOXXKITE_MCP_CATALOG(self) -> dict[str, str]:
        catalog: dict[str, str] = {}
        for entry in self.BOXXKITE_MCP_CATALOG_RAW.split(","):
            entry = entry.strip()
            if not entry:
                continue
    ...
    # mirrors BOXXKITE_MAX_SECRETS_PER_ACCOUNT's rationale (unbounded rows
    # per account is a quota gap, not just an API nicety).
    BOXXKITE_MAX_MCP_CONNECTIONS_PER_ACCOUNT: int = 50
    ...
    # BOXXKITE_WEBHOOK_RATE_LIMIT_PER_MINUTE's "own, more conservative bucket"
    # rationale.
    BOXXKITE_MCP_CONNECTION_RATE_LIMIT_PER_MINUTE: int = 10

    # ── Independent Storage Volumes (docs/EXTERNAL-STORAGE-MOUNTING-DESIGN.md's
    # Volume addendum) ────────────────────────────────────────────────────
    # Off by default, same rationale as BOXXKITE_IMAGE_BUILDER_ENABLED: a
    ...
    BOXXKITE_VOLUME_STORAGE_CLASS: str = "standard"
    ...
    BOXXKITE_VOLUME_PROVISION_TIMEOUT_SECONDS: int = 120

    # ── Background enforcement ───────────────────────────────────────────
    # How often the reaper task scans for sessions that have exceeded
    # BOXXKITE_MAX_SESSION_MINUTES and tears them down server-side.
    ...
    BOXXKITE_ADMIN_METRICS_MAX_ACCOUNTS: int = 500
    ...
    BOXXKITE_ADMIN_AUDIT_LOG_MAX_LIMIT: int = 500

    # ── Webhooks (docs/WEBHOOKS-DESIGN.md) ──────────────────────────────
    # Deliberately NO global on/off flag (unlike BOXXKITE_IMAGE_BUILDER_ENABLED/
    # BOXXKITE_VOLUMES_ENABLED above) -- this feature is per-account opt-in by
    ...
    # BOXXKITE_MAX_SECRETS_PER_ACCOUNT (unbounded rows per account is a quota
    # gap, not just an API nicety).
    BOXXKITE_MAX_WEBHOOKS_PER_ACCOUNT: int = 20
    ...
    BOXXKITE_WEBHOOK_DELIVERY_TIMEOUT_SECONDS: int = 10
    ...
    BOXXKITE_WEBHOOK_MAX_DELIVERY_ATTEMPTS: int = 6
    # Exponential backoff base: attempt N waits
    # min(BASE * 2**(N-1), BOXXKITE_WEBHOOK_RETRY_MAX_SECONDS) after the
    # previous attempt fails. Default schedule: 30s, 60s, 120s, 240s, 480s.
    BOXXKITE_WEBHOOK_RETRY_BASE_SECONDS: int = 30
    BOXXKITE_WEBHOOK_RETRY_MAX_SECONDS: int = 3600
    # How often the delivery worker polls for due WebhookDelivery rows --
    # mirrors BOXXKITE_SESSION_REAPER_INTERVAL_SECONDS's "simple poll loop,
    # not a new pub/sub system" posture.
    BOXXKITE_WEBHOOK_WORKER_INTERVAL_SECONDS: int = 5
    ...
    # BOXXKITE_ADMIN_METRICS_MAX_ACCOUNTS.
    BOXXKITE_WEBHOOK_WORKER_BATCH_LIMIT: int = 100
    ...
    BOXXKITE_WEBHOOK_RATE_LIMIT_PER_MINUTE: int = 10

    # ── Public demo playground (issue #103) ──────────────────────────────
    # Off by default, same "new public-facing surface stays opt-in"
    # convention as BOXXKITE_IMAGE_BUILDER_ENABLED/BOXXKITE_VOLUMES_ENABLED --
    ...
    BOXXKITE_DEMO_PLAYGROUND_ENABLED: bool = False
    ...
    # one ceiling.
    BOXXKITE_DEMO_MAX_CONCURRENT: int = 3
    ...
    # BOXXKITE_MAX_SESSION_MINUTES every real account gets.
    BOXXKITE_DEMO_LIFETIME_MINUTES: int = 4
    ...
    # default when no `subject` is given), same as BOXXKITE_AUTH_RATE_LIMIT_
    # PER_MINUTE's signup/login bucket.
    BOXXKITE_DEMO_RATE_LIMIT_PER_MINUTE: int = 3
    ...
    BOXXKITE_DEMO_EXEC_TIMEOUT_SECONDS: int = 10
