...
legal `catalog_id` values, and `settings.BOXXKITE_MCP_CATALOG` is the
...
class UnknownMcpCatalogEntryError(ValueError):
    ...
    `settings.BOXXKITE_MCP_CATALOG` -- guards against schemas.py's
    ...
    already guards against for `base`/`BOXXKITE_BASE_IMAGE_REFS`."""
...
def resolve_catalog_host(catalog_id: str) -> str:
    ...
    """
    host = settings.BOXXKITE_MCP_CATALOG.get(catalog_id)
    if not host:
        raise UnknownMcpCatalogEntryError(
            f"No catalog entry configured for catalog_id {catalog_id!r} -- check "
            "BOXXKITE_MCP_CATALOG_RAW/schemas.py's McpCatalogId Literal are in sync"
        )
    return host
