...
Configuration is read from the environment at import time (``BOXXKITE_BASE_URL``,
``BOXXKITE_API_KEY``) and the process fails fast with a clear error if either
is missing -- there is no sane default to silently fall back to.

Every tool catches ``BoxxkiteApiError``/``BoxxkiteConnectionError`` and returns
a descriptive string instead of raising, mirroring the defensive pattern in
``boxxkite_client.langchain_tools``: an uncaught exception here would otherwise
...
class ConfigurationError(RuntimeError):
    """Raised when required boxxkite MCP server configuration is missing."""


_LOCALHOST_HOSTNAMES = {"localhost", "127.0.0.1", "::1"}

# LSP CompletionItemKind enum (numeric codes the protocol actually sends,
# same mapping as src/boxxkite/tools/lsp_tools.py's _COMPLETION_ITEM_KIND_NAMES
# -- ported rather than imported since this package doesn't depend on the
# root boxxkite package, only boxxkite_client). Unknown codes fall back to
...
def _load_config() -> tuple[str, str]:
    ...
    missing = [
        name
        for name, value in (("BOXXKITE_BASE_URL", base_url), ("BOXXKITE_API_KEY", api_key))
        if not value
    ]
    if missing:
        raise ConfigurationError(
            ...
            + ", ".join(missing)
            + ". Set BOXXKITE_BASE_URL (your control-plane's base URL) and "
            "BOXXKITE_API_KEY (a bxk_live_... key from `boxxkite auth` or the "
            "hosted dashboard) before starting boxxkite-mcp."
        )
    assert base_url is not None and api_key is not None  # narrowed by the check above

    # BoxxkiteClient itself also validates this, but checking here first
    ...
    if not (is_https or is_local_http):
        raise ConfigurationError(
            f"BOXXKITE_BASE_URL={base_url!r} must use https:// (or http://localhost "
            "for local dev only) -- BOXXKITE_API_KEY is a full-privilege, long-lived "
...
def _package_version() -> str:
    """boxxkite-mcp's own package version, so the MCP handshake's
    ...
    try:
        return _dist_version("boxxkite-mcp")
    except PackageNotFoundError:
        return "0"


def _describe_api_error(action: str, exc: BoxxkiteApiError) -> str:
    return f"Error {action}: {exc.message} [{exc.code}] (HTTP {exc.status_code})"


def _describe_connection_error(action: str, exc: BoxxkiteConnectionError) -> str:
    return f"Error {action}: could not reach the boxxkite control-plane ({exc})"


def build_server(client: BoxxkiteClient) -> FastMCP:
    ...
    def create_sandbox(
        ...
        """Create a new boxxkite sandbox and return its session id and status.
    ...
    def create_sandbox_image(
        ...
        python_packages: packages to pip-install into the image. Each entry
            ...
            base="boxxkite-node", base="boxxkite-nextjs", base="boxxkite-go", or
            base="boxxkite-rust".
    ...
    def create_mcp_connection(label: str, catalog_id: str) -> str:
        ...
        catalog_id: which curated MCP catalog entry to grant -- one of
            "slack", "notion", "linear", "github". Restricted to boxxkite's
...
def main() -> None:
    """Entry point for the `boxxkite-mcp` console script -- reads config from
    the environment, fails fast if it's missing, then serves over stdio."""
    try:
        base_url, api_key = _load_config()
    except ConfigurationError as exc:
        print(f"boxxkite-mcp: {exc}", file=sys.stderr)
