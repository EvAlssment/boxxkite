...
dependencies. Nothing in this module is required to use boxxkite.
...
Pass nothing and boxxkite defaults to `NoOpAuditSink`, which records nothing
...
configuration needed. `SQLiteAuditSink` exists for the self-hosted `boxxkite`
...
class AuditSink(Protocol):
    ...
    `present_files`. You do not need to implement every method — boxxkite
...
class SQLiteAuditSink:
    ...
    Gives self-hosted users of the bare `boxxkite` package a working, durable
    ...
    a standalone `boxxkite` deployment, or replace it with your own
    `AuditSink` backed by Postgres/Redis/etc. for a distributed one.
    """

    def __init__(self, db_path: Union[str, Path] = "boxxkite_audit.db") -> None:
