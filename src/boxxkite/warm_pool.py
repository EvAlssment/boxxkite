...
# Start with: kubectl proxy --context kind-boxxkite-dev &
...
class ReadyPod:
    ...
    opt-in fast-claim path (BOXXKITE_FAST_CLAIM_ENABLED).
...
class WarmPoolManager:
    ...
    async def claim_pod(self, size: str = DEFAULT_SANDBOX_SIZE) -> Optional[tuple[str, str]]:
        ...
        try:
            pods = await self._k8s_core_api.list_namespaced_pod(
                ...
                label_selector="app=sandbox,pool=warm,sandbox.boxxkite.dev/status=warm",
            )
        except Exception as e:
            ...
            try:
                await self._k8s_core_api.patch_namespaced_pod(
                    ...
                    body=[
                        ...
                        {
                            "op": "test",
                            "path": "/metadata/labels/sandbox.boxxkite.dev~1status",
                            "value": "warm",
                        },
    ...
    # =========================================================================
    # Fast-claim ready-pod index (opt-in, BOXXKITE_FAST_CLAIM_ENABLED)
    # =========================================================================
        ...
        try:
            ...
            await self._k8s_core_api.patch_namespaced_pod(
                ...
                body={"metadata": {
                    ...
                    "annotations": {
                        "sandbox.boxxkite.dev/storage-prefix": None,
                        "sandbox.boxxkite.dev/upload-file-ids": None,
    ...
    async def _scan_pool_state(
        ...
        Returns:
            ...
            by_state:     Breakdown by sandbox.boxxkite.dev/status label.
        ...
        for pod in pods.items:
            ...
            status = labels.get("sandbox.boxxkite.dev/status") or "unknown"
            ...
            is_warm = (
                ...
                and labels.get("sandbox.boxxkite.dev/status") == "warm"
                and phase == "Running"
            )
    ...
    def _current_warm_pool_size_targets(self) -> dict[str, int]:
        ...
        BOXXKITE_ADAPTIVE_WARM_POOL_ENABLED is off (the default). When on,
    ...
    async def _replenish(self):
        ...
        2. Create fresh pods per size to bring warm_by_size back up to each
           ...
           BOXXKITE_ADAPTIVE_WARM_POOL_ENABLED is set -- see
    ...
    async def _reap_idle_pods(self):
        ...
        try:
            pods = await self._k8s_core_api.list_namespaced_pod(
                namespace=SANDBOX_NAMESPACE,
                label_selector="app=sandbox,sandbox.boxxkite.dev/status=claimed",
            )
        except Exception as e:
