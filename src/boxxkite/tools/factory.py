...
tools via boxxkite.tools.adapters.to_langchain_tools (requires the
`langchain` extra). Use boxxkite.tools.adapters.to_openai_functions(specs)
...
def create_sandbox_tool_specs(
    ...
    Args:
        ...
        enable_git_tools: Opt in to the git tool set (see
            src/boxxkite/tools/git_tools.py). Off by default -- clone/push/
        ...
        enable_http_request_tool: Opt in to the secrets-broker http_request
            tool (docs/SECRETS-DESIGN.md, src/boxxkite/tools/http_request_tool.py).
        ...
        enable_agent_pty: Opt in to the `pty_exec` tool
            ...
            AND separately requires `BOXXKITE_AGENT_PTY_ENABLED=true` on the
        ...
        enable_node_interpreter: Opt in to the `node_interpreter` tool
            ...
            `BOXXKITE_NODE_INTERPRETER_ENABLED=true` on the sidecar itself --
        ...
        enable_browser_tool: Opt in to the browser_navigate/browser_exec/
            ...
            (docs/BROWSER-EXEC-DESIGN.md, src/boxxkite/tools/browser_tools.py)
            -- a headless Chromium process driven via Playwright/CDP. Off
            by default, AND separately requires
            `BOXXKITE_BROWSER_ENABLED=true` on the sidecar itself -- this
            ...
            NetworkPolicy src/boxxkite/browser_network_policy.py builds for
        ...
        enable_lsp_tools: Opt in to the `lsp_start`/`lsp_completion`/
            ...
            src/boxxkite/tools/lsp_tools.py, GitHub issue #183) -- agent-
            ...
            `BOXXKITE_LSP_ENABLED=true` on the sidecar itself -- this
    ...
    if enable_agent_pty:
        ...
        # Also requires BOXXKITE_AGENT_PTY_ENABLED=true server-side -- this
    ...
    if enable_node_interpreter:
        ...
        # docstring above). Also requires BOXXKITE_NODE_INTERPRETER_ENABLED=true
    ...
    if enable_browser_tool:
        ...
        # BOXXKITE_BROWSER_ENABLED=true server-side -- this Python-layer flag
    ...
    if enable_lsp_tools:
        ...
        # docstring above). Also requires BOXXKITE_LSP_ENABLED=true
        # server-side -- this Python-layer flag alone does not turn the
...
def create_sandbox_tools(
    ...
    via boxxkite.tools.adapters.to_langchain_tools. Requires the
    `langchain` extra (`pip install boxxkite-sandbox[langchain]`).
