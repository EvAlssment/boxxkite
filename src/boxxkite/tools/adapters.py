...
- `to_openai_agents_tools`: `agents.tool.FunctionTool` objects (the OpenAI
  ...
  loop with boxxkite's tools. Requires the `openai-agents` extra; imported
...
def to_llamaindex_tools(specs: list[ToolSpec]) -> list:
    ...
    Requires the `llamaindex` extra: `pip install boxxkite-sandbox[llamaindex]`.
...
def _json_schema_to_pydantic_model(name: str, parameters: dict[str, Any]):
    ...
    `$ref`, etc), which no boxxkite tool currently needs.
    """
...
def to_openai_agents_tools(specs: list[ToolSpec]) -> list:
    ...
    Requires the `openai-agents` extra: `pip install boxxkite-sandbox[openai-agents]`.
