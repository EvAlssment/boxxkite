"""`boxxkite audit verify` -- read-only hash-chain verifier for a
`HashChainedSQLiteAuditSink` database file (GitHub issue #136,
docs/TAMPER-EVIDENT-AUDIT-DESIGN.md §7).

Local-only, unlike `boxxkite log`/`boxxkite watch`: this operates directly on
a SQLite file an operator points it at (their own `boxxkite_audit.db`, or a
...
from boxxkite.audit import verify_audit_db_file
...
def verify(
    db: Path = typer.Option(
        ...
        dir_okay=False,
        readable=True,
        help="Path to a HashChainedSQLiteAuditSink database file (e.g. boxxkite_audit.db).",
    ),
