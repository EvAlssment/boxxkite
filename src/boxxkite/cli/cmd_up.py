"""`boxxkite up` — replaces the README's manual `openssl rand -hex 32` +
...
(`~/.boxxkite/local.env`), and start the docker-compose stack with it.
"""
...
_BOXXKITE_COMPOSE_MARKERS = ("SIDECAR_AUTH_TOKEN", "sidecar")


def _looks_like_boxxkite_compose_file(path: Path) -> bool:
    try:
        content = path.read_text()
    except OSError:
        return False
    return all(marker in content for marker in _BOXXKITE_COMPOSE_MARKERS)
...
def _find_compose_file(explicit: Path | None, *, search_root: Path | None = None) -> Path:
    ...
    for _ in range(_COMPOSE_SEARCH_DEPTH):
        ...
        if compose_path.exists():
            if not _looks_like_boxxkite_compose_file(compose_path):
                raise CliError(
                    f"Found {compose_path}, but it doesn't look like a boxxkite compose file "
    ...
    raise CliError(
        "Could not find deploy/docker-compose.yml. Run `boxxkite up` from inside "
        "a boxxkite git checkout, or pass --compose-file <path>."
    )
...
def up(
    ...
    Generates a fresh SIDECAR_AUTH_TOKEN, writes it to ~/.boxxkite/local.env so
    `boxxkite exec` and `boxxkite files` pick it up automatically, then runs
    ...
    `boxxkite session --help` for why hosted mode is a separate thing.
    """
    ...
    typer.echo("")
    typer.secho("boxxkite is up.", fg=typer.colors.GREEN, bold=True)
    typer.echo(f"  Health check:   curl {sidecar_url}/health")
    typer.echo(f"  Token stored:   {LOCAL_ENV_FILE}")
    typer.echo("  boxxkite exec/files now talk to this stack automatically.")
    typer.echo("")
    typer.echo('Try it:   boxxkite exec "python3 -c \'print(1 + 1)\'"')
    typer.echo("")
    typer.echo(
        "Local docker-compose mode is a single exec/file-ops target — there is no "
        "multi-session management API here (see `boxxkite session --help`). For "
        "that, use `boxxkite signup` or `boxxkite config set-url`/`set-key` against "
