"""`boxxkite webhooks create/ls/rm/deliveries` — hosted-mode webhook
...
def _require_hosted(ctx: Context, command: str) -> None:
    if ctx.mode != "hosted":
        raise CliError(
            ...
            "(`boxxkite up`) has no webhook-delivery store of its own -- run `boxxkite signup` "
...
def create(
    ...
    typer.echo(
        ...
        f"X-Boxxkite-Webhook-Signature header with it): {result['secret']}"
    )
...
def rm(
    webhook_id: str = typer.Argument(..., help="Webhook ID to delete (see `boxxkite webhooks ls`)."),
) -> None:
    """Delete a webhook."""
    ctx = resolve_context()
