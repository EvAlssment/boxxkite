"""`boxxkite allowlist get/set/clear` — hosted-mode per-account command
allowlist management.

This is an OPT-IN guardrail, not a sandbox-escape boundary: it restricts
which command names (and, optionally, which argument patterns) `boxxkite
...
def get() -> None:
    ...
    An empty allowlist means unrestricted: `boxxkite exec` can run any
