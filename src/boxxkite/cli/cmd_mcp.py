"""`boxxkite mcp init <target>` — one-shot wire boxxkite's MCP server (the
`boxxkite-mcp` package, see mcp-server/) into an MCP-compatible client's own
config file, using the hosted base_url/api_key already saved by
`boxxkite signup` (or `boxxkite config set-url`/`set-key`).

This is the maintainer-suggested smallest slice of issue #219's broader
first-run-wizard request -- just the MCP-wiring step. The full `boxxkite init`
...
Either way, merge-write only ever touches the `boxxkite` entry -- any other
...
MCP_COMMAND = "boxxkite-mcp"
SERVER_NAME = "boxxkite"
...
def _merge_write(path: Path, *, base_url: str, api_key: str) -> bool:
    """Merge a `boxxkite` entry into `path`'s `mcpServers` block, leaving
    ...
    servers[SERVER_NAME] = {
        "command": MCP_COMMAND,
        "env": {
            "BOXXKITE_BASE_URL": base_url,
            "BOXXKITE_API_KEY": api_key,
        },
    }
...
def _merge_write_toml(path: Path, *, base_url: str, api_key: str) -> bool:
    """Same merge-only-touch-the-boxxkite-key contract as `_merge_write`, for
...
def init(
    ...
    """Wire boxxkite's MCP server into an MCP-compatible client's config file."""
    ...
    if not hosted.base_url or not hosted.api_key:
        raise CliError(
            "No hosted control-plane configured yet. Run `boxxkite signup`, or "
            "`boxxkite config set-url` and `boxxkite config set-key`, before `boxxkite mcp init`."
        )
    ...
    typer.secho(f"{verb} the boxxkite MCP server entry in {path}.", fg=typer.colors.GREEN)
