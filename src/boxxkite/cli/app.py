"""Wires all commands into the `boxxkite` typer app. Individual commands live
...
session_app = typer.Typer(
    help=(
        ...
        "(`boxxkite up`) starts a single exec/file-ops sidecar with no session API of its own."
    ),
    no_args_is_help=True,
)
...
allowlist_app = typer.Typer(
    help=(
        "Manage the account's command allowlist for `boxxkite exec` -- an opt-in "
...
volumes_app = typer.Typer(
    help=(
        ...
        "sandbox session via `boxxkite session create --volume-mounts`. Hosted mode only, "
...
mcp_app = typer.Typer(
    help=(
        "Wire boxxkite's MCP server into an MCP-compatible client's own config file "
        "(Claude Code, Cursor, Windsurf, Claude Desktop, Codex). Hosted mode only -- reads "
        "the base_url/api_key already saved by `boxxkite signup` or `boxxkite config`."
    ),
    no_args_is_help=True,
)
