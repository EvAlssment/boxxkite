"""`boxxkite volumes create/get/ls/rm` — hosted-mode independent storage
...
`session`/`secrets` use. Gated server-side by `BOXXKITE_VOLUMES_ENABLED` (off
...
single sandbox session -- mount one into a new session with `boxxkite session
...
def _require_hosted(ctx: Context, command: str) -> None:
    if ctx.mode != "hosted":
        raise CliError(
            ...
            "(`boxxkite up`) has no independent volume store of its own -- run `boxxkite signup` "
...
def create(
    ...
    """Queue creation of a storage volume. Always asynchronous -- poll `boxxkite volumes get <id>` for progress."""
    ctx = resolve_context()
...
def get(
    volume_id: str = typer.Argument(..., help="Volume ID to look up (see `boxxkite volumes ls`)."),
) -> None:
...
def rm(
    volume_id: str = typer.Argument(..., help="Volume ID to delete (see `boxxkite volumes ls`)."),
) -> None:
