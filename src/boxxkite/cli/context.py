...
(`boxxkite config set-url`/`set-key`, or `boxxkite signup`). Otherwise, if
`boxxkite up` has written a local sidecar token, local mode is used. If
...
def resolve_context() -> Context:
    ...
    raise CliError(
        ...
        "  - run `boxxkite config set-url <url>` and `boxxkite config set-key <key>`\n"
        "    (or `boxxkite signup`) to use a hosted control-plane."
    )
