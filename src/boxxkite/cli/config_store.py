...
- Hosted mode: ~/.boxxkite/config.toml (base_url + api_key), written by
  `boxxkite config set-url`/`set-key` or `boxxkite signup`.
- Local mode: ~/.boxxkite/local.env (SIDECAR_AUTH_TOKEN + SIDECAR_URL),
  written by `boxxkite up` after it starts the docker-compose stack.
...
def validate_base_url_scheme(base_url: str) -> None:
    ...
    full-privilege, long-lived account credential (src/boxxkite/cli/client.py's
...
def write_local_env(*, token: str, sidecar_url: str) -> None:
    ...
    LOCAL_ENV_FILE.write_text(
        "# Written by `boxxkite up` — read automatically by `boxxkite exec`/`boxxkite files`.\n"
