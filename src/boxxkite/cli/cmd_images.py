"""`boxxkite images build/get/ls/rm` — hosted-mode declarative custom image
builds (control-plane/src/control_plane/routers/images.py,
docs/DECLARATIVE-BUILDER-DESIGN.md).

Hosted-only, authenticated with the same long-lived API key `boxxkite exec`/
`session`/`secrets` use. Gated server-side by `BOXXKITE_IMAGE_BUILDER_ENABLED`
...
def _require_hosted(ctx: Context, command: str) -> None:
    if ctx.mode != "hosted":
        raise CliError(
            f"`boxxkite {command}` needs a hosted control-plane. Local docker-compose mode "
            "(`boxxkite up`) has no declarative image builder of its own -- run `boxxkite signup` "
...
def build(
    base: str = typer.Option(
        ...
        "--base",
        help=(
            "Pre-approved base image: boxxkite-default, boxxkite-minimal, boxxkite-node, "
            "boxxkite-go, boxxkite-nextjs, or boxxkite-rust."
        ),
    ),
    ...
    """Queue a custom image build. Always asynchronous -- poll `boxxkite images get <id>` for progress."""
    ctx = resolve_context()
...
def get(
    image_id: str = typer.Argument(..., help="Image ID to look up (see `boxxkite images ls`)."),
) -> None:
...
def rm(
    image_id: str = typer.Argument(..., help="Image ID to delete (see `boxxkite images ls`)."),
) -> None:
