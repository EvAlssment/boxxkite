"""`boxxkite secrets create/ls/rm` — hosted-mode org-scoped secret CRUD
...
def _require_hosted(ctx: Context, command: str) -> None:
    if ctx.mode != "hosted":
        raise CliError(
            ...
            "(`boxxkite up`) has no secrets-broker store of its own -- run `boxxkite signup` "
...
def rm(
    secret_id: str = typer.Argument(..., help="Secret ID to delete (see `boxxkite secrets ls`)."),
) -> None:
    """Delete a secret."""
    ctx = resolve_context()
