"""`boxxkite session create/ls/rm` — hosted-mode sandbox session management.

Deliberately hosted-only: local docker-compose mode (`boxxkite up`) starts a
single sidecar with no session concept of its own — there's nothing to list,
create, or remove beyond the one stack `boxxkite up` already started. Local
...
def _require_hosted(ctx: Context, command: str) -> None:
    if ctx.mode != "hosted":
        raise CliError(
            ...
            "mode (`boxxkite up`) has no session-management API — it's a single "
            "exec/file-ops sidecar, not a multi-session service. Run `boxxkite signup` "
...
def create(
    ...
    image_id: str | None = typer.Option(
        None,
        "--image-id",
        help="Id of a custom image built via `boxxkite images` (see POST /v1/images). "
        "Uses the operator's default image when omitted.",
    ),
    secret_names: list[str] | None = typer.Option(
        None,
        "--secret-names",
        help="Name of an account secret (see `boxxkite secrets`/POST /v1/secrets) to grant "
    ...
    volume_mounts: list[str] | None = typer.Option(
        None,
        "--volume-mounts",
        help="A volume_id=mount_path pair mounting an independent volume (see `boxxkite volumes`/"
    ...
    gpu_count: int | None = typer.Option(
        ...
        "422s unless the deployment has BOXXKITE_GPU_ENABLED set and a GPU-equipped node pool "
