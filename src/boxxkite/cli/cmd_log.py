"""`boxxkite log`/`boxxkite watch` — hosted-mode exec/file-op audit history for
...
table -- local docker-compose mode (`boxxkite up`) has no audit store of its
own, same reasoning as `boxxkite session`'s hosted-only restriction.
"""
...
def _require_hosted(ctx: Context, command: str) -> None:
    if ctx.mode != "hosted":
        raise CliError(
            ...
            "(`boxxkite up`) has no audit-log store of its own -- run `boxxkite signup` or "
