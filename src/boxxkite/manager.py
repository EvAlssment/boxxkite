...
class PodLifecycleMixin:
    async def _create_k8s_session(
        ...
        try:
            await self._k8s_core_api.patch_namespaced_pod(
                ...
                body={"metadata": {
                    ...
                    "annotations": {
                        "sandbox.boxxkite.dev/storage-prefix": s3_prefix,
                        "sandbox.boxxkite.dev/upload-file-ids": json.dumps(normalized_upload_file_ids),
        ...
        # (issue #74) -- no-op unless BOXXKITE_SECRETS_NETWORK_POLICY_ENABLED
        ...
        # BOXXKITE_BROWSER_NETWORK_POLICY_ENABLED is set. Same ordering
        ...
        _warn_if_browser_enabled_without_network_policy(
            session_id, browser_enabled, BOXXKITE_BROWSER_NETWORK_POLICY_ENABLED
        )
...
class SandboxManager(
    ...
    def __init__(self, *, session_metadata_store: Optional[SessionMetadataStore] = None):
        ...
        if self._use_docker_compose:
            ...
            # `boxxkite up` persists the sidecar token + URL to
            # ~/.boxxkite/local.env; load them here so library users and example
            # scripts don't have to re-export SIDECAR_AUTH_TOKEN/SIDECAR_URL by
            # hand after `boxxkite up`. Explicit env vars always win.
