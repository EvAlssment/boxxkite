...
Read src/boxxkite/checkpoint_backend.py's module docstring before touching
...
feature gate enabled. Off by default (BOXXKITE_FULL_STATE_SNAPSHOT_ENABLED).
"""
...
class FullStateCheckpointMixin:
    async def create_full_state_checkpoint(
        ...
        BOXXKITE_FULL_STATE_SNAPSHOT_ENABLED is unset (fail loud, matching
        ...
        if not full_state_snapshot_enabled():
            raise RuntimeError(
                "create_full_state_checkpoint was called but BOXXKITE_FULL_STATE_SNAPSHOT_ENABLED "
