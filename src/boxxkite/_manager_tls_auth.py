...
class TlsAuthMixin:
    ...
    async def _sync_secrets_egress_network_policy(
        ...
        NetworkPolicy (issue #74, src/boxxkite/secrets_network_policy.py),
        ...
        No-op entirely when BOXXKITE_SECRETS_NETWORK_POLICY_ENABLED is unset
        ...
        session creation.
        """
        if not BOXXKITE_SECRETS_NETWORK_POLICY_ENABLED:
            return

        await self._init_k8s()
        if not self._k8s_networking_api:
            logger.warning(
                "[SandboxManager] BOXXKITE_SECRETS_NETWORK_POLICY_ENABLED is "
    ...
    async def _sync_browser_egress_network_policy(
        ...
        src/boxxkite/browser_network_policy.py), replacing whatever egress
        ...
        No-op entirely when BOXXKITE_BROWSER_NETWORK_POLICY_ENABLED is unset
        (the default) -- this method must never be the reason a session
        ...
        """
        if not BOXXKITE_BROWSER_NETWORK_POLICY_ENABLED:
            return

        await self._init_k8s()
        if not self._k8s_networking_api:
            logger.warning(
                "[SandboxManager] BOXXKITE_BROWSER_NETWORK_POLICY_ENABLED is "
                "set but the K8s networking API is unavailable; skipping "
