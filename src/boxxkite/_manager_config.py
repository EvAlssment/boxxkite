...
``boxxkite.manager`` re-exports every name unchanged via
...
# BOXXKITE_IMAGE_BUILDER_ENABLED/BOXXKITE_AGENT_PTY_ENABLED): provisions one
...
# docs/SECRETS-DESIGN.md, src/boxxkite/secrets_network_policy.py). Requires
...
# codebase.
BOXXKITE_SECRETS_NETWORK_POLICY_ENABLED = (
    os.environ.get("BOXXKITE_SECRETS_NETWORK_POLICY_ENABLED", "false").lower() == "true"
)

# docs/BROWSER-EXEC-DESIGN.md §3 -- the per-session NetworkPolicy scoping
# mechanism for browser-enabled sessions (src/boxxkite/browser_network_policy.py).
# Same off-by-default, deliberate-operator-decision posture as
# BOXXKITE_SECRETS_NETWORK_POLICY_ENABLED above, but this one gates a
...
BOXXKITE_BROWSER_NETWORK_POLICY_ENABLED = (
    os.environ.get("BOXXKITE_BROWSER_NETWORK_POLICY_ENABLED", "false").lower() == "true"
)
...
def _warn_if_browser_enabled_without_network_policy(
    ...
    operator-level BOXXKITE_BROWSER_NETWORK_POLICY_ENABLED flag off fails
    ...
    if browser_enabled and not network_policy_enabled:
        logger.warning(
            ...
            f"{session_id} but BOXXKITE_BROWSER_NETWORK_POLICY_ENABLED is not set -- "
...
def _validate_gpu_count(gpu_count: Optional[int]) -> Optional[int]:
    ...
    silently ignored when BOXXKITE_GPU_ENABLED is off -- a caller asking for
    ...
    if not gpu_enabled():
        raise ValueError(
            "gpu_count was requested but BOXXKITE_GPU_ENABLED is not set -- this is an "
...
STORAGE_CREDENTIALS_SECRET = os.environ.get("STORAGE_CREDENTIALS_SECRET", "boxxkite-storage-credentials")

SAFE_TO_EVICT_ANNOTATION = "cluster-autoscaler.kubernetes.io/safe-to-evict"
SESSION_ID_ANNOTATION = "sandbox.boxxkite.dev/session-id"
ORGANIZATION_ID_ANNOTATION = "sandbox.boxxkite.dev/organization-id"
WORK_ITEM_ID_ANNOTATION = "sandbox.boxxkite.dev/work-item-id"
...
RESTORED_FROM_SNAPSHOT_ANNOTATION = "sandbox.boxxkite.dev/restored-from-snapshot"
