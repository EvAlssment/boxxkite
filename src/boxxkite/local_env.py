"""Read the ~/.boxxkite/local.env file that `boxxkite up` writes.
...
by hand after running `boxxkite up`.

Deliberately stdlib-only and free of any boxxkite imports so the core
...
def read_local_env_credentials(path: Path | None = None) -> tuple[str, str] | None:
    ...
    either value is missing. Defaults to the standard ~/.boxxkite/local.env."""
