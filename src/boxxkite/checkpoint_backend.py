...
document as boxxkite's core security property. This module never does
...
-- treat enabling BOXXKITE_FULL_STATE_SNAPSHOT_ENABLED with at least the
...
async def probe_checkpoint_support(core_api: client.CoreV1Api, node_name: str) -> bool:
    ...
    try:
        await core_api.connect_get_node_proxy_with_path(
            name=node_name, path="/checkpoint/__boxxkite_probe__/__boxxkite_probe__/__boxxkite_probe__"
        )
        return True
    except ApiException as exc:
        if exc.status == 404:
