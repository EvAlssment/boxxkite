...
from boxxkite_client import AsyncBoxxkiteClient, BoxxkiteApiError, BoxxkiteClient, RetryConfig
from boxxkite_client.client import _parse_retry_after, _retry_delay, _should_retry
...
def _client(handler, retry: RetryConfig | None = FAST_RETRY) -> BoxxkiteClient:
...
def _no_sleep(monkeypatch):
    slept: list[float] = []
    monkeypatch.setattr("boxxkite_client.client.time.sleep", lambda s: slept.append(s))

    async def _fake_asleep(s: float) -> None:
        slept.append(s)

    monkeypatch.setattr("boxxkite_client.client.asyncio.sleep", _fake_asleep)
    return slept
...
def test_gives_up_after_max_retries_and_raises():
    ...
    with pytest.raises(BoxxkiteApiError) as exc:
...
def test_does_not_retry_when_disabled():
    ...
    with pytest.raises(BoxxkiteApiError):
