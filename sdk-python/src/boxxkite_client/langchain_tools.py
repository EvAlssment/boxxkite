"""LangChain tool factory over a hosted boxxkite control-plane.

Mirrors boxxkite.tools' own tool names and shapes (bash_tool, file_create,
view, str_replace, ls, glob, grep) so an agent switching between a
self-hosted deployment (boxxkite.tools.create_sandbox_tools, embedding
...
`pip install boxxkite-client[langchain]`.
"""
...
from .client import BoxxkiteClient
from .exceptions import BoxxkiteApiError, BoxxkiteConnectionError


def create_sandbox_tools(client: BoxxkiteClient, session_id: str) -> list:
    ...
    Each tool catches BoxxkiteApiError and returns a descriptive string
    ...
    def bash_tool(command: str) -> str:
        ...
        except (BoxxkiteApiError, BoxxkiteConnectionError) as exc:
