"""LlamaIndex ReActAgent driving a boxxkite sandbox.

Uses boxxkite.tools.adapters.to_llamaindex_tools() to convert boxxkite's
...
§4.2), just against boxxkite's tool surface instead of a single
`execute_python` tool.
...
async def main() -> None:
    model_name = os.environ.get("BOXXKITE_EXAMPLE_MODEL", "gpt-5")
