...
(OpenAI, Anthropic, Mistral, Groq); boxxkite had the equivalent underlying
...
converts boxxkite's two `ToolSpec`s (`bash_tool`, `file_create`) into the
...
2. `pip install -e ../..` (boxxkite itself -- no extra needed, unlike the
...
call shape) were verified against the actual installed `boxxkite`/`openai`
