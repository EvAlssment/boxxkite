...
a `FunctionTool`, drive it from a `ReActAgent`); boxxkite had no LlamaIndex
adapter at all until `boxxkite.tools.adapters.to_llamaindex_tools` was added
...
report exactly what it printed. `to_llamaindex_tools()` converts boxxkite's
...
2. `pip install -e "../..[llamaindex]"` (boxxkite itself, with the
...
`boxxkite`/`llama-index-core`/`llama-index-llms-openai` package versions in
