...
boxxkite's `to_openai_agents_tools()` wraps its existing `ToolSpec`s as
...
converts boxxkite's two `ToolSpec`s (`bash_tool`, `file_create`) into
...
1. A running boxxkite stack: `boxxkite up` from the repo root.
2. `pip install -e "../..[openai-agents]"` (boxxkite itself, with the
...
`boxxkite`/`openai-agents` (0.18.2) package versions in this environment:
