...
docker compose \
  ...
  -f examples/browser-desktop/foundry_audit_sandbox/docker-compose.override.yml \
  up -d --build
```

(Verified for real: `docker build` from this directory succeeds standalone,
and `docker compose -f deploy/docker-compose.yml -f
examples/browser-desktop/foundry_audit_sandbox/docker-compose.override.yml build sandbox`,
