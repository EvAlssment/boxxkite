...
func TestDesktopTakeover_SendsAuthorizationHeaderAndBridgesBytes(t *testing.T) {
	upgrader := websocket.Upgrader{}
	var gotAuthHeader string

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/sandboxes/sess-1/desktop" {
	...
	conn, err := client.DesktopTakeover(context.Background(), "sess-1")
	if err != nil {
		t.Fatalf("DesktopTakeover: %v", err)
	}
	defer conn.Close()
...
func TestDesktopTakeover_ConnectionRefusedReturnsConnectionError(t *testing.T) {
	client, err := NewClient("http://localhost:1", "bxk_live_test")
	if err != nil {
		t.Fatalf("NewClient: %v", err)
	}

	_, err = client.DesktopTakeover(context.Background(), "sess-1")
	if err == nil {
