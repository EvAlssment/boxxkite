//! `desktop_takeover()` opens a raw WebSocket, so it can't be exercised
//! through `wiremock` (HTTP-only). Instead this spins up a bare `tokio` TCP
...
//! `Authorization: Bearer <api_key>` header exactly the way `takeover()`
...
#[tokio::test]
async fn desktop_takeover_sends_bearer_authorization_header_and_bridges_bytes() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        ws_stream
            .send(Message::text("hello from sandbox desktop"))
            .await
    ...
    let mut desktop_stream = client
        .desktop_takeover("sess_1")
        .await
        .expect("desktop_takeover should connect");
    let first_message = desktop_stream
        .next()
        .await
        .expect("should receive a message")
        .expect("message ok");
    assert_eq!(
        first_message.into_text().unwrap(),
        "hello from sandbox desktop"
    );
